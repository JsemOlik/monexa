...
export default function ManageComputersPage() {
  ...
  const handleRemoveRoom = async (id: any) => {
    if (!confirm("Are you sure you want to delete this room? Computers will be unassigned.")) return;
    try {
      await removeRoom({ id });
    } catch (error) {
  ...
  const activeComputer = activeId ? computers?.find(c => c._id === activeId) : null;
          ...
          <div className="flex flex-col gap-6 max-h-full">
            <div className="flex items-end justify-between">
              <div>
                ...
                <p className="text-muted-foreground text-sm">Organize your devices by dragging them between rooms.</p>
              </div>
              <form onSubmit={handleCreateRoom} className="flex gap-2 items-center">
                <Input
                  ...
                  className="w-48 h-9"
                />
                <Button size="sm" type="submit" disabled={isSubmitting || !newRoomName.trim()}>
            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar pb-10">
              ...
              >
                <div className="grid gap-6">
                  ...
                  {/* Unassigned section */}
                  <RoomSection
                    room={{ _id: "unassigned", name: "Unassigned Devices" } as any}
    ...
    <div className="flex flex-col gap-3">
      ...
      >
        <div className="flex items-center gap-2 flex-1">
          {isExpanded ? <IconChevronDown className="size-4 text-muted-foreground" /> : <IconChevronRight className="size-4 text-muted-foreground" />}
      ...
      {isExpanded && (
        <div 
          ref={setNodeRef}
          className={cn(
            "grid gap-4 min-h-[100px] p-4 rounded-2xl border border-dashed transition-colors",
            "@xs:grid-cols-1 @xl:grid-cols-2 @4xl:grid-cols-3 @6xl:grid-cols-4",
            computers.length === 0 ? "border-border/60 bg-muted/5" : "border-transparent bg-transparent"
          )}
        >
          {computers.length === 0 ? (
            <div className="col-span-full flex flex-col items-center justify-center py-4 text-center">
              ...
              <p className="text-xs text-muted-foreground italic">Drag devices here to assign to this room.</p>
            </div>
          ) : (
            <SortableContext items={computers.map(c => c._id)} strategy={rectSortingStrategy}>
              {computers.map(c => (
